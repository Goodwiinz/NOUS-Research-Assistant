...
resource "aws_nat_gateway" "main" {
  ...
  subnet_id     = aws_subnet.public[var.one_nat_gateway_per_az ? count.index : 0].id
...
resource "aws_route_table" "private" {
  count = var.enable_nat_gateway ? (var.one_nat_gateway_per_az ? length(var.private_subnets) : 1) : 0
...
resource "aws_security_group" "monitoring" {
  name_prefix = "${var.name}-monitoring-sg-"
